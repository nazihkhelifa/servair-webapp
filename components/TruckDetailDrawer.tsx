      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-base font-semibold text-gray-900">{truck.plateNumber || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Type</p>
                <p className="text-base font-semibold text-gray-900">{truck.type || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Model</p>
                <p className="text-base font-semibold text-gray-900">{truck.model || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Capacity</p>
                <p className="text-base font-semibold text-gray-900">{truck.capacity || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Status</p>
                <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(truck.status)}`}>
                  {truck.status}
                </span>
              </div>
              <div>
                <p className="text-sm text-gray-500">Assigned Driver</p>
                <p className="text-base font-semibold text-gray-900">{truck.assignedDriverId || 'None'}</p>
              </div>
            </div>
            {truck.notes && (
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <MdAirplanemodeActive className="h-5 w-5 text-blue-600" />
              Current Assignment
            </h3>
            {currentAssignment ? (
              <div className="space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="text-base font-semibold text-gray-900">{currentAssignment.title}</h4>
                    {currentAssignment.flightCode && (
                      <p className="text-sm text-blue-600 font-medium">Flight: {currentAssignment.flightCode}</p>
                    )}
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${getAssignmentStatusColor(currentAssignment.status)}`}>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{formatDate(currentAssignment.createdAt)}</p>
                  </div>
                  <div>
              ...
              <div className="text-center py-6">
                ...
                <p className="text-sm text-gray-400">Truck is available for new tasks</p>
              </div>
            )}
          </div>
            ...
            {trackingData ? (
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <FiWifi className="h-5 w-5 text-green-600 mt-0.5" />
                ...
                {trackingData.speedKmh !== undefined && trackingData.speedKmh !== null && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-semibold text-gray-900">{trackingData.speedKmh.toFixed(1)} km/h</span>
                  </div>
                )}
                {trackingData.driverId && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-semibold text-gray-900">{trackingData.driverId}</span>
                  </div>
                )}
                <a
              ...
              <div className="text-center py-6">
                ...
                <p className="text-sm text-gray-400">Truck hasn't reported position yet</p>
              </div>
            )}
          </div>
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        {assignment.flightCode && (
                          <p className="text-xs text-gray-600">Flight: {assignment.flightCode}</p>
                        )}
                      </div>
